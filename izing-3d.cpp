...
Izing3D::Izing3D(string _name, int _grid_size, int _mcs_max, int _statistic_max): Models(_name, _grid_size)  {
...
void Izing3D::WriteTdCharToFile()   {
    ...
    double E = 0, M = 0, C_v = 0, X = 0, relax = 0, correlation = 0, error_relax = 0, error_correlation = 0;
    ...
    for(int i = 0; i < streams; i++)   {
        ...
        buffer.C_v += data[i].sum[2];
        buffer.X += data[i].sum[3];
    ...
    C_v = (((buffer.C_v/(double)(mcs_max*(statistic_max))) - pow(buffer.M/(double)(mcs_max*(statistic_max)), 2.0))/T*T);
    X = (((buffer.X/(double)(mcs_max*(statistic_max))) - pow(buffer.E/(double)(mcs_max*(statistic_max)), 2.0))/T);
    ...
    cout << "( E = " << E << ", M = " << M << ", C_v = " << C_v << ", X = " << X << \
            ...
            " ( " << error_correlation << " ))" << endl;

    if(!out_appr.bad()) {

        out_appr << M << setw(10) << E << setw(10) << C_v << setw(10) << X << setw(10) << \
...
void Izing3D::Metropolis(double _T)    {
    ...
    #pragma omp parallel for schedule(dynamic,1) private(ci,cj,ck,r,spin_inverted,dE,W)
...
//    out_appr << fixed << setw(10) << data[thread_num].sum[1]/static_cast<double>(mcs_max*statistic_max) << endl;
//	out_appr << fixed << data[thread_num].sum[0]/static_cast<double>(mcs_max*statistic_max) << setw(10) << data[thread_num].sum[1]/static_cast<double>(mcs_max*statistic_max);
//	out_appr << fixed << setw(10) << ((data[thread_num].sum[2]/static_cast<double>(mcs_max*statistic_max)) - pow(data[thread_num].sum[0]/static_cast<double>(mcs_max*statistic_max), 2.0))/T*T;
//	out_appr << fixed << setw(10) << ((data[thread_num].sum[3]/static_cast<double>(mcs_max*statistic_max)) - pow(data[thread_num].sum[1]/static_cast<double>(mcs_max*statistic_max), 2.0))/T;
//	out_appr << fixed << setw(10) << T << endl;
...
//      int j;  // j coordinate
    ...
    //     for(int j=0;j<N;j++)    {
    //         for(int k=0;k<N;k++)    {
    //             if(Mersenne.IRandomX(0,999)/1000.0 < 0.2)   {
            ...
            do {
                ...
                element_k.pop();
                    ...
                    if(ci!=0)    {
                        ...
                        if(spin[ci-1][cj][ck]==-spin[ci][cj][ck])   {
                        ...
                        if(spin[N-1][cj][ck]==-spin[ci][cj][ck])   {
                    ...
                    if(cj!=0)   {
                        ...
                        if(spin[ci][cj-1][ck]==-spin[ci][cj][ck])   {
                        ...
                        if(spin[ci][N-1][ck]==-spin[ci][cj][ck])    {
                    ...
                    if(ck!=0)   {
                        ...
                        if(spin[ci][cj][ck-1]==-spin[ci][cj][ck]) {
                        ...
                        if(spin[ci][cj][N-1]==-spin[ci][cj][ck])   {
...
void Izing3D::WolfMassiveBased(double _T)   {
    ...
    #pragma omp parallel for schedule(static,1) private(ci,cj,ck,r,W,cluster_size,cluster,n_cl,n_cl_1,n_cl_2)
    for(int start_conf = 0; start_conf < statistic_max; start_conf++)   {
